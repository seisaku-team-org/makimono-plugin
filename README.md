...
```
/makimono:find      # 巻物を探して読み込む (消費)
/makimono:publish   # 手元の指示書をマキモノに出品する (供給)
```
...
**publish(供給)** — 「このCLAUDE.mdを出品して」で起動:
1. 機密(APIキー・顧客名等)を除去し、メタデータを起案
2. 人間の承認を得てから出品API (`/api/v1/listings`) に送信
3. 審査後にマキモノで公開される
